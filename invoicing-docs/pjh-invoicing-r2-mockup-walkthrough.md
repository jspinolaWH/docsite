...
This version reflects the rebuilt mockup, where four areas were taken from thin stubs to full feature shape: the New manual event form (PD-283), the New billing run form (PD-291/290/292/293), the New fee assignment form (PD-288), and the Invoice options card (PD-300/301/302/307/308/294/289/271). Each now shows the real mechanic with the scope boundary still visible, pricing and accounting shown as auto-resolved not typed, the surcharge off, the language fixed to FI.
...
**On the view:** the Cycle column in the run list (Monthly, Quarterly, Immediate), and the Billing cycle field at the top of the New billing run form (Monthly, Quarterly, Semi-annual, Annual, Immediate).
...
**On the view:** the Bundling preview card, where 12 emptyings collapse to one line while the pallet stays separate, and the Bundling section of the New billing run form, where each product group (emptying services, base and seasonal fees, reception, equipment rental) is set to bundle into one line or list individually.
**Works with:** it shapes what the invoice from a run looks like, feeding the line structure that PD-310's FINVOICE then carries. The bundled events stay separate records underneath; only the invoice line is consolidated.
...
**On the view:** RUN-003, "Immediate / sludge events on completion / separate invoice," and the two restriction toggles in the New billing run form (bill on a separate invoice, invoice immediately on completion).
**Works with:** it is the exception to PD-291's regular cycle, same run list, different rule. The sludge use case is those two flags on the sludge service.

### [PD-293](https://ioteelab.atlassian.net/browse/PD-293) Run filtering
**Requirement:** "It must be possible to define different limit conditions for printing the invoice. The customer must be able to bill only a part of the customer's transactions," then lists municipality, € threshold, invoicing rhythm, period, customer type, service type, reception location, service responsibility.
**On the view:** the Run filters card showing the summary as badges, and the Selection filters section of the New billing run form, which carries all eight named conditions (municipality, minimum € threshold, invoicing frequency, time period, customer type, service type, reception location, service responsibility). The form is where the full named set lives; the filters card is the at-a-glance summary on a run.
**Works with:** it is the selector that decides which events enter a run before PD-290 bundles them. Under-threshold invoices carry to the next run rather than being dropped.
...
**On the view:** the Status column and the left sidebar (Draft / Ready to send / Sent / Error).
**Works with:** this is the state machine the whole module references, the same lock that greys out a sent event's Edit button on the detail page. The New billing run form creates a run in Draft, and the footnote on the form says so.
...
**On the view:** the "+ New manual event" button opens a form built to the full event shape, the same sections as an event's detail page. Event and customer (event date and customer entered by hand, with customer number, municipality, service responsibility and contractor resolving from the customer); Product and quantity (the product/service plus the operational fields: quantity, weight for weight-based products, registration number, place of unloading, origin, direction); Pricing and accounting, shown greyed and read-only with the note "resolved from product + price list on save"; and Comments.
**Works with:** the greyed pricing and accounting section is the visual proof of the requirement, a manual event inherits accounts, cost centre and VAT from the product exactly like an automated one. The operational fields are typed, the money is derived, and on save the event runs PD-278 validation, so it then flows through the same PD-296 / PD-310 plumbing as any other event.
...
**Works with:** this is the literal expression of PD-297's lock, and a saved edit re-runs PD-278 validation. Pricing and accounting stay read-only in edit mode, the same boundary the manual-event form draws.
...
**On the view:** the fee assignment table (annual base fee for permanent residences in January, holiday homes in April, shared-point monthly), and the New fee assignment form, where you pick the fee product, what it applies to (a property class or a specific contract), the frequency and the invoiced month or cadence. The Amount is read-only and sourced from the price list.
**Works with:** the scheduler generates these as ordinary events, so they then behave like any PD-299 event in a run. The read-only amount and the form's footnote make the boundary explicit, amount changes are price-list changes, not a fee engine.
...
This one pink card is where the invoice-shaping requirements sit together, because they all describe how a single invoice is produced. In the rebuilt view each requirement is its own block showing the actual mechanic, not a one-line summary.
...
**On the view:** the Reverse-charge VAT block, an example line at 0% VAT with the buyer's VAT number (FI17829345) and the statutory text "Käännetty verovelvollisuus AVL 8 c §," with a note that on this public-law invoice it is not applicable and the block shows the market-based case.
**Works with:** it reads the product's reverse-charge flag, so a B2B reception event (like ev3) produces a 0% line with the statutory reference; it is the R2 item that fixes the R1 reception-VAT gap. Squad footnote: the ticket's example still says 24% VAT, the standard rate is 25.5%.
...
**On the view:** the Gross / net block, a VAT summary per rate (base, VAT, total) with a Gross/Net toggle set to Gross.
...
**On the view:** the Custom invoice text block, with actual InvoiceFreeText (header) and RowFreeText (line) input fields, the answer to Päivi's open @mention.
**Works with:** the single-invoice (customer-specific) case is on the card. The bulk case is requirement text ("in bulk") and is not yet drawn on the view; its home is a run-level text field on the New billing run form, applied to every invoice in the run. Build note for the squad: do not treat the card alone as full PD-302 coverage, the run-level field still needs a home.
...
**On the view:** the Template block, a template-code field (e.g. PJH-DEFAULT-01) with the note that templates themselves are managed in the operator's system.
...
**On the view:** the Language block, a fixed FI selector (disabled) with the note that PJH does not use per-customer language selection.
**Works with:** it is the one requirement displayed as deliberately not built for PJH, the single sign-off item, shown honestly rather than hidden. Still needs Päivi's written confirmation before the squad treats it as closed.
...
**On the view:** the Surcharge block, a greyed surcharge line (Laskutuslisä, 0.00 €) with the global toggle shown Off.
...
**On the view:** the AR / reporting data block, a sample line carrying the public/private classification, accounting account, cost centre and VAT.
**Works with:** this is the compliance payload that R1 already stamps on events via PD-296, verified here as complete on the invoice line and in the FINVOICE export.
...
**On the view:** two places, the Pre-send checks block on this card (a pass/fail checklist: billing address present, e-invoice address valid, totals reconcile, FINVOICE mandatory fields complete), and RUN-004 sitting in Error because checks found 3 invoices missing a valid e-invoice address.
...
|---|---|---|
| [PD-291](https://ioteelab.atlassian.net/browse/PD-291) | Different billing rhythms | Billing runs, Cycle column + New run form |
| [PD-290](https://ioteelab.atlassian.net/browse/PD-290) | Bundle transactions onto lines | Billing runs, Bundling preview + New run form |
| [PD-292](https://ioteelab.atlassian.net/browse/PD-292) | Invoice some events separately | Billing runs, RUN-003 + New run form restrictions |
| [PD-293](https://ioteelab.atlassian.net/browse/PD-293) | Limit conditions for a run | Billing runs, Run filters card + New run form (8 filters) |
...
| [PD-283](https://ioteelab.atlassian.net/browse/PD-283) | Create events manually | Events, + New manual event (full event form) |
...
| [PD-288](https://ioteelab.atlassian.net/browse/PD-288) | Time-based fees | Seasonal fees, assignment table + New fee form |
| [PD-286](https://ioteelab.atlassian.net/browse/PD-286) | Minimum charge | Seasonal fees, Minimum charge card |
| [PD-300](https://ioteelab.atlassian.net/browse/PD-300) | Reverse-charge VAT | Invoice detail, Invoice options block |
| [PD-301](https://ioteelab.atlassian.net/browse/PD-301) | Gross or net | Invoice detail, Invoice options block |
| [PD-302](https://ioteelab.atlassian.net/browse/PD-302) | Custom + bulk text | Invoice detail, Invoice options (single invoice; bulk not yet on view) |
| [PD-307](https://ioteelab.atlassian.net/browse/PD-307) | Choose invoice template | Invoice detail, Invoice options block |
...
| [PD-289](https://ioteelab.atlassian.net/browse/PD-289) | Mandatory AR data on lines | Invoice detail, Invoice options block |
| [PD-271](https://ioteelab.atlassian.net/browse/PD-271) | Automatic checks | Invoice detail + RUN-004 (Error) |

---

*Document updated: 2026-06-16 · Ledger squad · WasteHero invoicing module*
*Requirements near-quoted from the Jira ticket "Requirement" sections (re-read 2026-06-10). The view referenced is the rebuilt R1+R2 mockup. Displayed at MVP level; not yet implemented.*
