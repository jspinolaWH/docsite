...
import { useLinearCycles, useLinearCycleIssues, getCycleStatus } from '../hooks/useLinearCycles';
...
function CycleDetailModal({ cycle, onClose }: { cycle: LinearCycle; onClose: () => void }) {
  const { issues, loading: issuesLoading, error: issuesError } = useLinearCycleIssues(cycle.id);
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '2rem 1rem', overflowY: 'auto' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--color-surface)', borderRadius: '12px', width: '100%', maxWidth: '920px', boxShadow: '0 24px 64px rgba(0,0,0,0.35)', overflow: 'hidden', marginBottom: '2rem' }}>
        ...
        {/* Loading / error state for issues */}
        {issuesLoading && (
        ...
        {issuesError && (
          <div style={{ padding: '1rem 1.5rem', color: '#991b1b', fontSize: '0.9rem' }}>
        ...
        {!issuesLoading && !issuesError && (
          <>
            {/* Filters */}
            <div style={{ padding: '0.875rem 1.5rem', borderBottom: '1px solid var(--color-border)', display: 'flex', flexWrap: 'wrap', gap: '0.6rem' }}>
            ...
            {/* Column headers */}
            <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr 150px 120px 140px', gap: '0.75rem', padding: '0.5rem 1.5rem', borderBottom: '1px solid var(--color-border)', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-secondary)' }}>
...
function CycleCard({ cycle, onClick }: { cycle: LinearCycle; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderLeft: `4px solid ${color}`, borderRadius: '8px', padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', opacity: status === 'completed' ? 0.75 : 1, cursor: 'pointer', transition: 'box-shadow 0.15s, transform 0.15s' }}
    ...
    >
      ...
      <div style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>
        {pct}% complete · click to see issues
      </div>
    </div>
  );
}
                ...
                return (
                  <section key={status} style={{ marginBottom: '2.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
                      {list.map((c) => <CycleCard key={c.id} cycle={c} onClick={() => setSelected(c)} />)}
                    </div>
                  </section>
                );
              })}
